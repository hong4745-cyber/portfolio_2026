    ...
    <section id="about" aria-label="About" className="relative min-h-[100svh] overflow-hidden bg-[#f4f0e8] text-[#0c1742]">
      <div className="mx-auto grid min-h-[100svh] w-full max-w-[1500px] grid-cols-1 items-center gap-8 px-8 py-16 md:grid-cols-[1fr_1.25fr_1fr] md:gap-0 md:px-16 md:py-20">
        ...
        <div className="relative flex h-[52vh] min-h-[360px] items-end justify-center md:h-[74vh]">
          <motion.div initial={{ scale: 0.86, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.9 }} className="absolute bottom-0 left-1/2 aspect-square w-[min(78vw,620px)] -translate-x-1/2 rounded-full bg-[#ffd21c]" />
          <motion.img src={personImage} alt="Baek Jieun portrait" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.15 }} className="relative z-10 h-[108%] w-auto max-w-none object-contain object-bottom" />
        </div>
